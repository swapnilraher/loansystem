  ...
  const visibleRemarks = React.useMemo(() => {
    return remarks.filter(remark => {
      ...
      if (isAutomatedAction) {
        return false // System action click logs are for Admin only
      }
      return true
    })
  }, [remarks])
          ...
          <div className="flex flex-col gap-2 md:items-end border-t md:border-t-0 border-slate-100 pt-4 md:pt-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="text-left md:text-right">
                <p className="text-sm font-black text-primary">₹{parseInt((lead.status === 'Disbursed' || lead.status === 'Disbursement Approval Pending' ? lead.disbursedAmount : null) || lead.amount || "0").toLocaleString()}</p>
                {(lead.status === 'Disbursed' || lead.status === 'Disbursement Approval Pending') && (
                  <p className="text-[10px] font-bold text-slate-400 mt-0.5">Disbursed amount (डिस्बर्स रक्कम)</p>
                )}
              </div>
            </div>
            {lead.bankName && (
              <div className="text-xs font-bold text-slate-400 mt-1">
                Bank: <span className="text-secondary font-black">{lead.bankName}</span>
              </div>
            )}
            <div className="mt-2">
              <select 
