...
import { ShieldCheck, Lock, ArrowRight, CheckCircle2, Car } from "lucide-react"
...
export function CarLoanForm() {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem("car_loan_draft")
    if (savedData) {
  ...
  useEffect(() => {
    localStorage.setItem("car_loan_draft", JSON.stringify(formValues))
  ...
  const onSubmit = async (data: LeadFormData) => {
    try {
      console.log("Saving Car Loan Lead via API:", data)
      const response = await fetch('/api/leads', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...data,
          source: "Website - Car Loan",
          type: "Car Loan",
        }),
      });
      ...
      localStorage.removeItem("car_loan_draft")
      setStep(3) // Success
    } catch (error: any) {
      ...
      {step === 1 && (
        <>
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h3 className="font-black text-secondary text-xl">Car Loan Eligibility</h3>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button
              type="submit"
              className="w-full h-11 text-xs font-black uppercase tracking-wider bg-gradient-to-r from-orange-500 to-red-550 hover:from-orange-600 hover:to-red-650 border-none text-white"
              size="md"
            ...
            <p className="text-[10px] text-center text-muted-foreground mt-4 leading-relaxed font-medium">
              By clicking, you agree to our Terms & Conditions and authorize us to contact you for car loan options.
            </p>
          </form>
        </>
      )}
        ...
        <div className="text-center space-y-8 py-10 animate-in fade-in zoom-in duration-500">
          ...
          <div className="space-y-3">
            ...
            <p className="text-slate-500 font-medium leading-relaxed">
              Your application has been received. Our auto finance expert will call you within 15 minutes.
            </p>
          </div>
          <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Application ID</p>
            <p className="text-xl font-mono font-black text-secondary">TS-CL-{Math.floor(Math.random()*90000) + 10000}</p>
          </div>
          <Button 
